...
namespace MGE
{
	public class HitboxCollider : Collider
	{
		...
		public HitboxCollider(Vector2 size, Vector2? position = null) : base(position)
		{
			_size = size;
		}

		public bool Intersects(HitboxCollider hitbox) => absLeft < hitbox.absRight && absRight > hitbox.absLeft && absBottom > hitbox.absTop && absTop < hitbox.absBottom;
		public bool Intersects(float x, float y, float width, float height) => absRight > x && absBottom > y && absLeft < x + width && absTop < y + height;

		protected override void Draw(Color color) => GFX.DrawRectInwards(bounds, color);
