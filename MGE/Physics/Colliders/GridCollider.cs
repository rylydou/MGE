...
namespace MGE
{
	public class GridCollider : Collider
	{
		...
		public GridCollider(float cellSize, string mapStr, Vector2? position = null) : base(position)
		{
			this.cellSize = cellSize;
		...
		public GridCollider(float cellSize, bool[,] data, Vector2? position = null) : base(position)
		{
			this.cellSize = cellSize;
		...
		public GridCollider(float cellSize, VirtualMap<bool> data, Vector2? position = null) : base(position)
		{
			this.cellSize = cellSize;
		...
		protected override void Draw(Color color)
		{
			for (int x = 0; x < cellsX; x++)
				for (int y = 0; y < cellsY; y++)
					if (data[x, y])
						GFX.DrawRectInwards(new Rect(absLeft + x * cellSize, absTop + y * cellSize, cellSize, cellSize), color);
		}
		...
		{
			if (collider is HitboxCollider hitbox)
