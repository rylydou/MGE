...
#pragma warning restore CA2014 // Do not use stackalloc in loops
			...
			else if (Check("IDAT", fourbytes))
			{
				...
				{
					int size = SysMath.Min(idatChunk.Length, chunkLength - i);
...
#pragma warning restore CA2014 // Do not use stackalloc in loops
		...
		// It would be cool to do this line-by-line so we don't need to create a buffer to store the decompressed stream
		{
			...
			{
				int lineLength = (int)SysMath.Ceiling((width * components * depth) / 8f);
				int bpp = SysMath.Max(1, (components * depth) / 8);
				int dest = 0;
				...
				for (int y = 0; y < height; y++)
				{
					...
					{
						Array.Copy(buffer, source, buffer, dest, SysMath.Min(bpp, lineLength));
					...
					else if (lineFilter == 4)
					{
						...
						{
							for (int x = 0, c = SysMath.Min(bpp, lineLength); x < c; x++)
							{
								buffer[dest + x] = (byte)(buffer[source + x] + buffer[dest + x - lineLength]);
							}
	...
	public static unsafe bool Write(Stream stream, int width, int height, Color[] pixels)
	{
		...
		static void WriteIDAT(BinaryWriter writer, MemoryStream memory, bool writeAll)
		{
			...
			{
				var amount = SysMath.Min(remainder, MaxIDATChunkLength);
			...
			using (DeflateStream deflate = new DeflateStream(zlibMemory, CompressionLevel.Fastest, true))
			{
				fixed (Color* ptr = pixels)
				{
					...
					for (int y = 0; y < height; y++)
					{
						...
						{
							var segment = new Span<byte>(pixelBuffer + x * 4, SysMath.Min(width - x, MaxHorizontalStep) * 4);
	...
	static byte PaethPredictor(byte a, byte b, byte c)
	{
		...
		int pb = SysMath.Abs(p - b);
		int pc = SysMath.Abs(p - c);
